...
def get_all_xg(Team, verbose=False):
    broken_NamesSC = [
        "Game_SJE_BIF_Score_2_2_Day_2024-11-24Z.pkl",
        "Game_SJE_VFF_Score_2_2_Day_2024-09-01Z.pkl",
        "Game_SJE_FCN_Score_1_4_Day_2024-10-06Z.pkl",
    ]
                    ...
                    if len(idxs) != 0:  # time of XG is 'wrong;
                        ...
                        while Ball[idx][5] == 0:
                            ...
                            if k == 25:
                                if verbose:
                                    print(
                                        "Cannot find the ball being played in this second"
                                    )
                                idx = idxs[0]
                                sign = -1
                            elif k > 75:
                        ...
                        if passed:
                            if len(Us[idx]) < 11 or len(Them[idx]) < 11:
                                ...
                                sign_ball_x = np.sign(Ball[idx][0])
                                row = {
                                    "XG": XGValue[i],
                                    "Time": XGTimes[i],
                                    "Half": XGHalf[i],
                                    "ball_x": sign_ball_x * Ball[idx][0],
                                    "ball_y": Ball[idx][1],
                                    "ball_z": Ball[idx][2],
                                    "ball_speed": Ball[idx][3],
                                }
                                for j in range(11):
                                    row[f"us_{j+1}_x"] = sign_ball_x * Us[idx][j][0]
                                    row[f"us_{j+1}_y"] = Us[idx][j][1]
                                    row[f"us_{j+1}_speed"] = Us[idx][j][3]
                                for j in range(11):
                                    row[f"them_{j+1}_x"] = sign_ball_x * Them[idx][j][0]
